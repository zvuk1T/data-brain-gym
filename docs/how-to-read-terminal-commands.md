...
## ANATOMY OF A COMMAND — CHECK IF FILE EXISTS
...
```
cat .gitignore  →  "Read and print the contents of this file"  📄
```

- If the file exists → prints its contents to the terminal
- If the file does not exist → produces an error on **channel 2 (stderr)**

---

### `2>/dev/null` — silence the error

```
2>/dev/null  →  "If there's an error, throw it into the void"  🕳️
```

- `2` = stderr (error channel)
- `>` = redirect to...
- `/dev/null` = a special "black hole" on macOS/Linux — anything sent here disappears silently

**Why we use it:**
Without this, if `.gitignore` doesn't exist, the terminal prints:
```
cat: .gitignore: No such file or directory
```
That's noise. We already handle the "not found" case with `||` — so we silence the raw error.
...
```
command1 || command2  →  "Try #1. If it FAILS — run #2 instead"  🔀
```
...
```
echo "text"  →  "Print this text to the terminal"  🖨️
```

- `echo` is the simplest output command — it just prints whatever you give it
- Used here as a human-readable fallback message

---

### COMPLETE MENTAL MODEL — file existence check

```
cat .gitignore     →  "Read the file"                              📄
2>/dev/null        →  "Silence any error (throw it in the void)"   🕳️
||                 →  "If that failed..."                          🔀
echo "NOT FOUND"   →  "...print this message instead"             🖨️
```
